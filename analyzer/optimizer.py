...
class UHECROptimizer(object):
    ...
    def __init__(self, single_results, spectrum, Xmax, ncoids = None, Emin=6e9, norms=None):
        self.Emin = Emin
        ...
        if norms is None:
            norms = np.ones_like(self.lst_res)
        if ncoids is None:
    ...
    def compute_combined_result(self, norms, deltaE=0.):
        ...
        self._extract_solver_result(result_comb,deltaE)
    ...
    def _extract_solver_result(self, res, deltaE):
        _, self.res_spectrum = res.get_solution_group(
            ...
            egrid=self.egrid_spectrum*(1-deltaE))

        _, mean_lnA, sigma_lnA = res.get_lnA(
            [el for el in res.known_species if el >= 100],
            egrid=self.egrid_xmax*(1-deltaE))
        self.res_xmax = self.XmaxModel.get_mean_Xmax(mean_lnA, self.egrid_xmax*(1-deltaE))
        self.res_var_xmax, _ = np.sqrt(
            self.XmaxModel.get_sigma2_Xmax(mean_lnA, sigma_lnA**2,
                                           self.egrid_xmax*(1-deltaE)))
    ...
    def get_chi2_spectrum(self, norms=None):
        ...
        error = np.where(
            self.res_spectrum[sl] > self.spectrum['spectrum'][sl],
    ...
    def fit_data_minuit(self, spectrum_only=False, minimizer_args={}, start_val=None):
        ...
        init_norm = self.spectrum['spectrum'][14] / self.res_spectrum[14] / len(self.ncoids)
        ...
        params.update({'error_'+name: val for name, val in zip(arg_names, error)})
        params.update({'limit_'+name: val for name, val in zip(arg_names, limit)})

        m = Minuit(
            chi2,
            forced_parameters = arg_names,
            **params
        )
        m.migrad()
        return m
...
class UHECRWalker(object):
    ...
    def lnprob_mc(self,
        ...
        optimizer = UHECROptimizer(
            lst_models, self.spectrum, self.xmax, Emin=Emin,ncoids=particle_ids)
        ...
        if return_blob:
            ...
            return minres.fval, (self.extract_minuit(minres), [res.to_dict() for res in optimizer.lst_res])
        else:
            return minres.fval
